...
SELECT * FROM procesion ORDER BY FIELD(dia_semana, 'Lunes Santo', 'Martes Santo', 'Miércoles Santo', 'Jueves Santo', 'Viernes Santo', 'Sábado Santo', 'Domingo de Ramos');
...
SELECT COUNT(*) AS total_procesiones FROM procesion WHERE barrio = 'Albaicín';
...
SELECT antiguedad, COUNT(*) AS total_hermandades FROM hermandad WHERE numero_costaleros > 100 GROUP BY antiguedad;

-- 29. Obtener el número de procesiones que tienen puntos de paso después de las 20:00.
SELECT id_procesion, COUNT(*) AS total_puntos FROM itinerario WHERE hora_paso > '20:00:00' GROUP BY id_procesion;

-- 30. Agrupar las hermandades por antigüedad y contar cuántas hay en cada rango.
SELECT antiguedad, COUNT(*) AS total FROM hermandad GROUP BY antiguedad;

-- 31. Concatenar el nombre y barrio de cada procesión en un único campo, separados por un guion.
SELECT CONCAT_WS(' - ', nombre, barrio) AS procesion_info FROM procesion;
...
SELECT nombre, DATE_FORMAT(hora_inicio, '%r') AS hora_12h FROM procesion;

-- 39. Mostrar solo la hora (sin minutos ni segundos) del paso por cada punto del itinerario.
SELECT id_procesion, TIME_FORMAT(hora_paso, '%H') AS solo_hora FROM itinerario;
